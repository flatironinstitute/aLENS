...
UNIT_CUBE_VERTEX_POSITIONS = np.array([[0., 0.,0.],
                                        ...
                                        [1., 0.,1.],
                                        [1., 1.,1.]])
ROT_UNIT_CUBE_VERT_POS = Rotation.from_rotvec(np.pi/4 * np.array([0., 0., 1.])).apply(UNIT_CUBE_VERTEX_POSITIONS)
ROT_CUBE_VERT_POS = Rotation.from_rotvec(np.arctan(np.sqrt(2)) * np.array([1., 0., 0.])).apply(ROT_UNIT_CUBE_VERT_POS)
...
def get_bead_cube_pos_from_orig_bead_info(pos, direction, radius):
    ...
    vertex_pos = Rotation.from_rotvec(np.random.rand() * 2. * np.pi * np.array([0., 0., 1.])).apply(vertex_pos)
    vertex_pos = Rotation.from_rotvec(polar_angle * np.array([0., 1., 0.])).apply(vertex_pos)
    vertex_pos = Rotation.from_rotvec(azimuthal_angle * np.array([0., 0., 1.])).apply(vertex_pos)
